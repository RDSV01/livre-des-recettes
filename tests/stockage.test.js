...
test('un fichier corrompu bloque le démarrage au lieu d’être écrasé', (t) => {
  const dossier = dossierTemporaire();
  t.after(() => fs.rmSync(dossier, { recursive: true, force: true }));

  fs.writeFileSync(path.join(dossier, 'livre-des-recettes.json'), '{ pas du json', 'utf8');
  assert.throws(() => creerStockage(dossier), /illisible/);
});
